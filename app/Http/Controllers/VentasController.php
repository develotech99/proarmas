...
public function procesarVenta(Request $request): JsonResponse
{
    ...
    try {
        ...
        foreach ($request->productos as $productoData) {
            ...
            if (!$producto) {
                throw new \Exception("Producto con ID {$productoData['producto_id']} no encontrado");
            }
            ...
            if (!$stockActual || $stockActual->stock_cantidad_disponible < $productoData['cantidad']) {
                throw new \Exception("Stock insuficiente para el producto: {$producto->producto_nombre}");
            }
            ...
            if ($productoData['requiere_serie'] == 1) {
                ...
                if (empty($seriesSeleccionadas)) {
                    throw new \Exception("El producto {$producto->producto_nombre} requiere series");
                }

                if (count($seriesSeleccionadas) !== $productoData['cantidad']) {
                    throw new \Exception("Debe seleccionar exactamente {$productoData['cantidad']} serie(s) para {$producto->producto_nombre}");
                }
                ...
                if ($seriesInfo->count() !== count($seriesSeleccionadas)) {
                    throw new \Exception("Una o más series no están disponibles para el producto {$producto->producto_nombre}");
                }
                ...
                if ($productoData['tiene_lotes'] && !empty($productoData['lotes_seleccionados'])) {
                    ...
                    if ($totalAsignado !== $productoData['cantidad']) {
                        throw new \Exception("La cantidad asignada en lotes ({$totalAsignado}) debe coincidir con la cantidad del producto ({$productoData['cantidad']}) para {$producto->producto_nombre}");
                    }
                    ...
                    foreach ($lotesSeleccionados as $loteData) {
                        ...
                        if (!$lote || $lote->lote_cantidad_disponible < $loteData['cantidad']) {
                            throw new \Exception("El lote {$lote->lote_codigo ?? $loteData['lote_id']} no tiene suficiente stock disponible");
                        }
