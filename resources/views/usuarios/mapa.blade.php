...
<style>
    ...
    #map { height: 520px; }
    #map_card:fullscreen #map,
    #map_card.in-fullscreen #map { height: 100% !important; }
    ...
    body:has(#map_card.in-fullscreen) { overflow: hidden !important; }
    body.fullscreen-active { overflow: hidden !important; }

    #mobile-sidebar { z-index: 10; }
    ...
    #map_card.in-fullscreen #mobile-sidebar { display: none !important; }
</style>
...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold tracking-tight bg-gradient-to-r from-slate-900 to-slate-700 dark:from-slate-100 dark:to-slate-300 bg-clip-text text-transparent">
            ...
            <div class="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                <span class="inline-flex items-center"><span class="w-2 h-2 rounded-full bg-emerald-500 mr-2"></span>Visitado/Comprado</span>
                <span class="inline-flex items-center"><span class="w-2 h-2 rounded-full bg-amber-500 mr-2"></span>Visitado/No comprado</span>
                <span class="inline-flex items-center"><span class="w-2 h-2 rounded-full bg-red-500 mr-2"></span>No visitado</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid gap-8 grid-cols-1 lg:grid-cols-3">
        ...
        <div class="min-w-0">
            <form id="FormRegistroUbicaciones"
                ...
                {{-- Cliente --}}
                <div class="mb-5">
                    <label for="cliente_id" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Cliente</label>
                    <select id="cliente_id" name="cliente_id"
                        ...
                        @foreach ($usuarios as $op)
                            <option value="{{ $op->id }}">{{ $op->name }} | {{ $op->user_empresa ?? 'Sin Empresa' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                {{-- Dirección --}}
                <div class="mb-5">
                    <label for="direccion" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Dirección / referencia</label>
                    <textarea id="direccion" name="direccion" rows="3"
                              placeholder="Ej. 6a ave 1-23 zona 1, frente al parque…"
                ...
                {{-- Estado visita --}}
                <div class="mb-5">
                    <label for="visitado" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Estado de la Visita</label>
                ...
                <div id="grupo_fecha_visita" class="mb-5">
                    <label for="fecha_visita" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Fecha de la Visita</label>
                ...
                {{-- Buscar lugar --}}
                <div class="mb-5">
                    <label for="busqueda_lugar" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Buscar lugar</label>
                    <div class="relative">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                    <div>
                        <label for="lat" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Latitud</label>
                        <input id="lat" name="lat" type="text" placeholder="14.6…"
                               class="w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
                    </div>
                    <div>
                        <label for="lng" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Longitud</label>
                ...
                <div id="venta_info"
                    ...
                    <h3 class="text-sm font-semibold text-emerald-800 dark:text-emerald-300 mb-3">Información de Venta</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="cantidad_vendida" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Cantidad Vendida</label>
                            <input id="cantidad_vendida" name="cantidad_vendida" type="number"
                                   class="w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
                        </div>
                        <div class="sm:col-span-2">
                            <label for="descripcion_venta" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Descripción de la Venta</label>
                ...
                <div class="flex flex-col sm:flex-row sm:flex-wrap gap-3">
                    ...
                    <button id="btn_guardar" type="submit"
                        ...
                        {{-- floppy --}}
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M17 3H7a2 2 0 00-2 2v14a2 2 0 002 2h10a2 2 0 002-2V5a2 2 0 00-2-2zm-5 14a3 3 0 110-6 3 3 0 010 6z"/>
                        </svg>
                        Guardar
                    </button>
                </div>
            ...
            <div id="map_card"
                ...
                <div class="relative">
                    ...
                    <button id="btn_fullscreen" type="button"
                        ...
                        <!-- expandir -->
                        <svg id="icon_expand" class="h-5 w-5 block text-slate-700 dark:text-slate-200" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/>
                        </svg>
                        <!-- contraer -->
                        <svg id="icon_compress" class="h-5 w-5 hidden text-slate-700 dark:text-slate-200" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div class="px-4 py-3 text-sm text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
    ...
    <div class="mt-10">
        <div class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg">
            <div class="p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Clientes Georreferenciados</h3>
