...
@section('title', 'Gestión de Métodos de Pago')
...
@json($metodosPago->items())
</script>

<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" 
     x-data="metodosPagoManager()">
            ...
            <div class="p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Métodos</dt>
                            <dd class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ $metodosPago->total() }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ $metodosPago->where('metpago_situacion', 1)->count() }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ $metodosPago->where('metpago_situacion', 0)->count() }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg mb-6">
        <div class="px-4 py-5 sm:p-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Buscar método</label>
                    <input type="text" 
                           x-model="searchTerm" 
                           @input="filterMetodos()"
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($metodosPago as $metodo)
                                <tr x-show="showMetodo({{ $metodo->metpago_id }})" x-transition>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        {{ $metodo->metpago_id }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                            {{ $metodo->metpago_descripcion }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                            {{ $metodo->metpago_situacion == 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}"
                                              data-status="{{ $metodo->metpago_situacion }}">
                                            {{ $metodo->metpago_situacion == 1 ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <button @click="editMetodo({{ $metodo->metpago_id }})" 
                                                    class="text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </button>
                                            <button @click="deleteMetodo({{ $metodo->metpago_id }})" 
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="mt-2">No hay métodos de pago registrados.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showModal"
                ...
                <form @submit="handleFormSubmit($event)">
                    <div class="mb-4">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100" x-text="isEditing ? 'Editar Método de Pago' : 'Crear Nuevo Método de Pago'"></h3>
                    </div>

                    <div class="grid grid-cols-1 gap-4">
                        <!-- Descripción -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Descripción del método</label>
                            <input type="text" 
                                   x-model="formData.metpago_descripcion"
                                   required
                                   maxlength="50"
                                   @input="validateForm()"
                        ...
                        <div>
                            ...
                            <select x-model="formData.metpago_situacion"
                                    required
                                    @change="validateForm()"
                                    class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md shadow-sm focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm">
...
@vite('resources/js/metodos-pago/index.js')